...
function validIPAddress(IP: string): string {
  function validIPv4Address(IP: string): string {
    const nums = IP.split('.');
    for (const num of nums) {
      if (num.length === 0 || num.length > 3) return 'Neither';
      if (num.length !== 1 && num.charAt(0) === '0') return 'Neither';
      if (parseInt(num, 10).toString() !== num) return 'Neither';
      if (parseInt(num, 10) > 255 || parseInt(num, 10) < 0) return 'Neither';
    }
    return 'IPv4';
  }
  function validIPv6Address(IP: string): string {
    const str = '0123456789abcdefABCDEF';
    const nums = IP.split(':');
    for (const num of nums) {
      if (num.length === 0 || num.length > 4) return 'Neither';
      for (let index = 0; index < num.length; index++) {
        if (str.indexOf(num.charAt(index)) === -1) return 'Neither';
      }
    }
    return 'IPv6';
  }
  if (IP.split('').filter((char) => char === '.').length === 3) return validIPv4Address(IP);
  if (IP.split('').filter((char) => char === ':').length === 7) return validIPv6Address(IP);
  return 'Neither';
}
