...
function shortestToChar(s: string, c: string): number[] {
  const result = new Array(s.length).fill(Number.MAX_SAFE_INTEGER / 2);
  let prevIndex = undefined;
  for (let i = 0; i < s.length; i++) {
    if (s.charAt(i) === c) prevIndex = i;
    if (prevIndex !== undefined) {
      result[i] = Math.min(result[i], Math.abs(i - prevIndex));
    }
  }
  prevIndex = undefined;
  for (let i = s.length - 1; i >= 0; i--) {
    if (s.charAt(i) === c) prevIndex = i;
    if (prevIndex !== undefined) {
      result[i] = Math.min(result[i], Math.abs(prevIndex - i));
    }
  }
  return result;
}
